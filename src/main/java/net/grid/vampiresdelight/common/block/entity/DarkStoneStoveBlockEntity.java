...
public class DarkStoneStoveBlockEntity extends SyncedBlockEntity {
    ...
    public static void cookingTick(Level level, BlockPos pos, BlockState state, DarkStoneStoveBlockEntity stove) {
        boolean isStoveLit = state.getValue(DarkStoneStoveBlock.LIT);
    ...
    public static void animationTick(Level level, BlockPos pos, BlockState state, DarkStoneStoveBlockEntity stove) {
        for (int i = 0; i < stove.inventory.getSlots(); ++i) {
            if (!stove.inventory.getStackInSlot(i).isEmpty() && level.random.nextFloat() < 0.2F) {
                Vec2 stoveItemVector = stove.getStoveItemOffset(i);
                Direction direction = state.getValue(DarkStoneStoveBlock.FACING);
